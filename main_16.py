...
def main():
    image = Image.open('Drowning_Princess.png')
    pixels = image.load()
    ...
    # 16 colors for 8 bits per pixel output image
    palette = octree.make_palette(16)

    # create palette for 16 color max and save to file
    palette_image = Image.new('RGB', (4, 4))
    palette_pixels = palette_image.load()
    for i, color in enumerate(palette):
        palette_pixels[i % 4, i / 4] = (int(color.red), int(color.green), int(color.blue))
    ...
    palette_out.save('D_palette_16.png')
